...
class plane(object):
    ...
    def __init__(self, x, y):
        ...
        self.height = 52
        self.hitbox = (self.x, self.y, self.width, self.height) # to determine if plane has been hit
...
class user_plane(plane):
    ...
    def draw(self, win, is_shield):
        ...
        win.blit(self.image, (self.x, self.y)) # blit the main plane on window at its x and y coordinates
        self.hitbox = (self.x, self.y, self.width, self.height) # redefine hit box
...
class enemy_plane(plane):
    ...
    def __init__(self, x, y, velocity):
        ...
        self.image = pygame.transform.scale(self.image, (self.width, self.height)) # change image size
    ...
    def draw(self, win):
        ...
        self.hitbox = (self.x, self.y, self.width, self.height) # redefine hitbox every time plane moves
        pygame.draw.rect(win, (255,0,0), (self.hitbox[0], self.hitbox[1] - 15, self.hitbox[2], 10)) # health bar
...
class bullet(projectile):
    ...
    def __init__(self, x, y, direction=1, angle=0):
        ...
        self.velocity = 15 * self.direction # bullets move at velocity 15, and in the same direction as the vessel that shot it
...
class tank(object):
    ...
    image = pygame.image.load("tank.png") # image displayed to represent tank
    ...
    def __init__(self, x, y, facing):
        ...
        self.hitbox = (self.x, self.y, self.width, self.height) # to determine if tank has been hit by a projectile
        self.velocity = 3 * facing # velocity multiplied by facing to ensure it moves in correct direction
        self.image_left = pygame.transform.scale(self.image, (self.width, self.height)) # left version of image
        self.image_right = pygame.transform.flip(self.image_left, True, False) # right version is flipped
    ...
    def draw(self, win):
        ...
        self.hitbox = (self.x, self.y, self.width, self.height) # redefine hitbox each time the tank moves
    ...
    def move(self):
        ...
        if self.velocity > 0: # if tank moving right
            if self.x + self.velocity < self.path[1]: # check if tank has reached edge of path
            ...
            if self.x - self.velocity > self.path[0]: # check if tank has reached edge of path
                self.x += self.velocity
            else: # if tank has reached edge of screen change its velocity and direction it is facing
                self.velocity = self.velocity * -1
                self.facing = -self.facing
...
class blimp(object):
    ...
    image = pygame.image.load('blimp.png') # upload icon to represent blimp
    ...
    def __init__(self, x, y):
        ...
        self.hitbox = (self.x, self.y, self.width, self.height) # to determine if blimp is hit with user bullet
        self.velocity = 10 # initial speed of blimp
        self.image = pygame.transform.scale(self.image, (self.width, self.height)) # resize image to match widthand height variables
    ...
    def draw(self, win):
        ...
        self.hitbox = (self.x, self.y, self.width, self.height) # redefine hitbox each time it moves
    ...
    def fire(self):            
        ...
        enemy_bullets.append(bullet(round(self.x), round(self.y + self.height//2), -1, 0)) # horizontal fire
        enemy_bullets.append(bullet(round(self.x), round(self.y + self.height//2), -1, .3)) # sligthly upward angled fire
        enemy_bullets.append(bullet(round(self.x), round(self.y + self.height//2), -1, -.3)) # sligthly downward angled fire
...
class message(object):
    ...
    def __init__(self):
        ...
        self.left_button_box = (450, 260, self.button_width, self.button_height) # hitboxes for left and right buttons
...
class collectible(object):
    ...
    def __init__(self, x, y):
        ...
        self.y = y
        self.hitbox = (self.x - self.radius, self.y - self.radius, self.radius * 2, self.radius * 2) # to determine if collectible has been collected
    ...
    def draw(self, icon, win):
        ...
        pygame.draw.circle(win, (0,0,0), (self.x, self.y), self.radius, 2) # blit circle border 
        win.blit(icon, (self.x - 25, self.y - 25)) # blit icon in the centre of circle
        ...
        if self.hitbox[0] < main_plane.hitbox[0] + main_plane.hitbox[2]:
            if self.hitbox[0] + self.hitbox[2] > main_plane.hitbox[0]:
                if self.hitbox[1] < main_plane.hitbox[1] + main_plane.hitbox[3]:
                    if self.hitbox[1] + self.hitbox[3] > main_plane.hitbox[1]: # determines if plane has entered hitbox of collectible 
...
class extra_life(collectible):
    ...
    heart = pygame.image.load('heart.png') # for icon

    def __init__(self, x, y):
        collectible.__init__(self, x, y) # call initialisation of superclass
        self.icon = pygame.transform.scale(self.heart, (self.width, self.height)) # resize icon
...
def redraw_game_window(score):
    ...
    text = font.render('Score: ' + str(score), 1, (0,0,0)) # display score to user
    win.blit(text, (50,50)) # blit score on screen

    if main_plane.visible:
        main_plane.draw(win, is_shield) # draw user controlled plane if it hasn't been destroyed
    
    for enemy in enemies: # iterate through all enemies
        enemy.draw(win) # draw each enemy
        main_plane.is_collide(enemy) # check if main_plane has collided with each enemy
        for bullet in bullets: # for each bullet and each enemy check if bullet has hit enemy
            if bullet.is_hit(enemy):
                bullets.pop(bullets.index(bullet)) # if enemy hit, remove bullet from list
                enemy.hit() # activate hit function if enemy hit
        if enemy.visible == False: # don't display enemy if destroyed
            enemies.pop(enemies.index(enemy)) # remove enemy from list if destroyed
    ...
    for bullet in enemy_bullets: # blit each enemy bullet on screen
        bullet.draw(win)
        if main_plane.is_hit(bullet): # for each enemy bullet check if user has been hit. is_hit contains necessary changes to user if user is hit
            enemy_bullets.pop(enemy_bullets.index(bullet)) # if user is hit remove enemy bullet from list
        ...
        if boss.visible:
            ...
            for i in bullets: # for each user bullet
                if i.is_hit(boss): # check if boss is hit
                    bullets.pop(bullets.index(i)) # if bullet hits, make it dissapear
...
def user_movement(main_plane, screen_x, screen_y, bullets, bombs, is_rapid_fire):
    ...
    if bullet_limit > 0: # to prevent bullet firing being too fast
        bullet_limit += 1
    if bullet_limit > 5 or (bullet_limit > 3 and is_rapid_fire): # timer is shorter when rapid fire is active
        bullet_limit = 0
...
def produce_enemy():
    ...
    for enemy in enemies:
        if random.randint(0, 50) == 25: # 1 in 50 chance of enemy firing bullet
            enemy_bullets.append(bullet(round(enemy.x), round(enemy.y + enemy.height//2), -1)) # enemy plane fires
...
bg = pygame.image.load('inf_background.jpg') # load in background image
bg_width = bg.get_width() # get background width
bg = pygame.transform.scale(bg, (bg_width, screen_y)) # make background correct height
...
while run:
    ...
    while level1 and run: # only runs while level1 and run is True
        ...
        while intromessage and level1 and run: # if main loop broken or level1 broken, break out of loop
            ...
            intromessage = message_obj.screen_message(win, message_obj.home, message_obj.home_play, message_obj.home_exit) # display first level message
        ...
        for i in bullets:
            if i.move() == False: # move user bullets
                bullets.pop(bullets.index(i)) # and remove if they make impact or reach edge of screen
        for i in enemy_bullets: 
            if i.move() == False: # move enemy bullets
                enemy_bullets.pop(enemy_bullets.index(i)) # and remove if they make impact or reach edge of screen
        user_movement(main_plane, screen_x, screen_y, bullets, bombs, is_rapid_fire) # to allow user to move planee

        if random.randint(0,500) == 100 and not collectibles: # 1 in 500 chance of new collectible appearing, and no more than one at a time
            choice = random.randint(0,2) # randomly choose collectible to appear
            if choice == 0: 
                collectibles.append(extra_life(random.randint(50,1000), random.randint(50,700)))
            elif choice == 1:
                collectibles.append(rapid_fire(random.randint(50,1000), random.randint(50,700)))
            else:
                collectibles.append(shield(random.randint(50,1000), random.randint(50,700)))
    ...
    main_plane.lives = 3
    tanks = [tank(-130, 700, 1), tank(2000, 700, -1)] # new variable for this level - tanks
    ...
    while level2 and run:
        ...
        while intromessage and level2 and run: # if main loop broken or level2 broken, break out of loop
            ...
            intromessage = message_obj.screen_message(win, message_obj.level2, message_obj.level2_play, message_obj.level2_exit) # display second level message
            pygame.display.update() # manually refresh window display
        ...
        for i in bombs: 
            if i.move() == False: # move user bombs
                bombs.pop(bombs.index(i)) # and remove if impact made or hit ground
        user_movement(main_plane, screen_x, screen_y, bullets, bombs, is_rapid_fire) # allow user to move plane
    ...
    while level3 and run: # only runs while level3 and run is True
        ...
        while intromessage and level3 and run: # if main loop broken or level1 broken, break out of loop
            ...
            intromessage = message_obj.screen_message(win, message_obj.level3, message_obj.level3_play, message_obj.level3_exit) # display third level message
            pygame.display.update() # manually refresh window display
        ...
        for i in bombs:
            if i.move() == False: # move user bombs
                bombs.pop(bombs.index(i)) # and remove if they make impact or reach edge of screen
        user_movement(main_plane, screen_x, screen_y, bullets, bombs, is_rapid_fire) # to allow user to move planee
    ...
    if main_plane.gameover == True: # if user loses
        ...
        while intromessage and run: # run will break loop if user presses close
            ...
            intromessage = message_obj.screen_message(win, message_obj.lost, message_obj.lost_play, message_obj.lost_exit) # display "you lost" message
            pygame.display.update() # manually refresh window display
        ...
        while intromessage and run: # run will break loop if user closes window
            ...
            intromessage = message_obj.screen_message(win, message_obj.won, message_obj.won_play, message_obj.won_exit) # display "you won" message
            pygame.display.update() # manually refresh window display
